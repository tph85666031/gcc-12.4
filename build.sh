...
if [ x"$MODE_ENCRYPT" == x'true' ];then
    ...
    find ${DIR_ROOT}/build/gcc-12.4/libexec/gcc/ -name cc1plus | xargs rm > /dev/null 2>&1
    find ${DIR_ROOT}/build/gcc-12.4/libexec/gcc/ -name g++-mapper-server | xargs rm > /dev/null 2>&1
